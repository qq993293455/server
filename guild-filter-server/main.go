...
import (
	...
	_ "coin-server/guild-filter-server/env"
	"coin-server/rule"
	...
	"coin-server/guild-filter-server/service"
...
func main() {
	...
	log := logger.MustNewAsync(zap.DebugLevel, &logger.Options{
		Console:    "stdout",
		RemoteAddr: nil,
		FilePath:   []string{fmt.Sprintf("./%s.%d.log", models.ServerType_GuildFilterServer.String(), serverId)},
		InitFields: map[string]interface{}{
			"serverType": models.ServerType_GuildFilterServer,
			"serverId":   serverId,
		},
		Development: true,
	})
	...
	utils.Must(err)
	tracing.Init(cnf, serverId, models.ServerType_GuildFilterServer)
	...
	s := service.NewService(urls, log, serverId, models.ServerType_GuildFilterServer)
	go func() {
		s.Serve()
	}()
