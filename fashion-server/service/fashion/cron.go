...
type OwnerCron struct {
	tree   *rbtree.RbTree
	owners map[values.RoleId]map[values.FashionId]*Cron
	mu     sync.Mutex
...
func (oc *OwnerCron) AddCron(cron *Cron) {
	if cron == nil || cron.RoleId == "" || cron.FashionId <= 0 {
		return
	}
	if cron.When <= 0 {
	...
	if oc.OwnerCronExist(cron) {
		oc.removeCron(cron)
		if _, ok := oc.owners[cron.RoleId]; ok {
			delete(oc.owners[cron.RoleId], cron.FashionId)
		}
	}
	if _, ok := oc.owners[cron.RoleId]; !ok {
		oc.owners[cron.RoleId] = map[values.FashionId]*Cron{}
	}
	oc.owners[cron.RoleId][cron.FashionId] = cron
...
func (oc *OwnerCron) OwnerCronExist(cron *Cron) bool {
	owner, ok := oc.owners[cron.RoleId]
	if !ok {
		return false
	}
	item, ok := owner[cron.FashionId]
	if !ok {
		return false
	}
...
func (oc *OwnerCron) run(cron *Cron) {
	if err := oc.pool.Submit(func() {
		...
		oc.logger.Error("cron exec error", zap.Error(err), zap.String("role_id", cron.RoleId))
	}
}
