...
type Cron struct {
	Id         values.Integer
	ActivityId values.Integer
	OrderKey   int64
	When       values.TimeStamp
	RetryTimes values.Integer
	Exec       func(id values.Integer) // 执行器
}
...
type OwnerCron struct {
	tree   *rbtree.RbTree
	owners map[values.Integer]*Cron
	mu     sync.Mutex
...
func InitOwnerCron(logger *logger.Logger) {
	pool1, err := ants.NewPool(10000)
	if err != nil {
		panic(fmt.Sprintf("ants.NewPool failed: %s", err.Error()))
	}
	pool2, err := ants.NewPool(10000)
	if err != nil {
		panic(fmt.Sprintf("ants.NewPool failed: %s", err.Error()))
	}
	endOC = &OwnerCron{
...
func (oc *OwnerCron) AddCron(cron *Cron) {
	if cron == nil || cron.Id <= 0 || cron.ActivityId <= 0 {
		return
	}
	if cron.When <= 0 {
	...
	if oc.OwnerCronExist(cron.Id) {
		oc.removeCron(cron.Id)
		delete(oc.owners, cron.Id)
	}
	oc.owners[cron.Id] = cron
...
func (oc *OwnerCron) OwnerCronExist(id values.Integer) bool {
	item, ok := oc.owners[id]
	if !ok {
		return false
	}
...
func (oc *OwnerCron) removeCron(id values.Integer) {
	item, ok := oc.owners[id]
	if !ok {
		return
	}
...
func (oc *OwnerCron) run(cron *Cron) {
	if err := oc.pool.Submit(func() {
		if cron.ActivityId > 0 {
			cron.Exec(cron.ActivityId)
		}
	}); err != nil {
		oc.logger.Error("cron exec error", zap.Error(err), zap.Int64("activity_id", cron.ActivityId))
	}
}
