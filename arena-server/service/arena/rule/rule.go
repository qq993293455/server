...
func GetAllRobot(ctx *ctx.Context) []rulemodel.PvPRobot {
	configs := rule.MustGetReader(ctx).PvPRobot.List()[0:]
	return configs
}

func GetRobot(ctx *ctx.Context, id int64) (*rulemodel.PvPRobot, bool) {
	config, ok := rule.MustGetReader(ctx).PvPRobot.GetPvPRobotById(values.Integer(id))
	if !ok {
		return nil, ok
	}
	return config, ok
...
func GetPvpRankReward(ctx *ctx.Context, id int64) (*rulemodel.PvPRankReward, bool) {
	config, ok := rule.MustGetReader(ctx).PvPRankReward.GetPvPRankRewardById(id)
	if !ok {
		return nil, ok
	}
	return config, ok
...
func GetRobotName(ctx *ctx.Context) (string, bool) {
	nameList := rule.MustGetReader(ctx).RobotName.List()
	size := len(nameList)
	if size == 0 {
		return "", false
	}
	...
	return nameList[firstIndex].FirstName + " " + nameList[secondIndex].SecondName, true
}
...
func GetArenaOpeningDuration(atype models.ArenaType, log *logger.Logger) (int64, *errmsg.ErrMsg) {
	if atype == models.ArenaType_ArenaType_Default {
		config := GetArenaDefalutPublicInfo(ctx.GetContext(), log)
		if config == nil {
			return 0, errmsg.NewErrArenaConfig()
		}
		return int64(config.ArenaOpeningDuration), nil
	}
	return 0, errmsg.NewErrArenaType()
}
