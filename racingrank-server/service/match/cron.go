...
func (oc *OwnerCron) AddCron(cron *Cron) {
	...
	if oc.OwnerCronExist(cron.RoleId) {
		oc.removeCron(cron.RoleId)
		delete(oc.owners, cron.RoleId)
	}
	oc.owners[cron.RoleId] = cron
...
func (oc *OwnerCron) OwnerCronExist(roleId values.RoleId) bool {
	item, ok := oc.owners[roleId]
	if !ok {
		return false
	}
...
func (oc *OwnerCron) removeCron(roleId values.RoleId) {
	item, ok := oc.owners[roleId]
	if !ok {
		return
	}
