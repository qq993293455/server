...
type Element struct {
	...
	name string             /*唯一名称*/
	val  interface{}        /*分值，可重复*/
}

func (e *Element) GetVal() interface{} {
	return e.val
}
...
type SkipList struct {
	...
	maxLevel int                    /*当前最大层数*/
	elements map[string]interface{} /*元素列表*/
	compare  Compare
}
...
func NewSeed(seed int64, compare Compare) *SkipList {
	...
	headNode := &Element{
		...
		prev: nil,
		name: HeadNodeName,
		val:  &models.RankValue{OwnerId: HeadNodeName, Value1: math.MaxInt64},
	}
	...
	tailNode := &Element{
		...
		prev: nil,
		name: TailNodeName,
		val:  &models.RankValue{OwnerId: TailNodeName, Value1: math.MinInt64},
	}
...
/*插入新节点*/
func (t *SkipList) Insert(name string, value interface{}) {
...
func (t *SkipList) Get(name string) (interface{}, bool) {
	value, ok := t.elements[name]
	return value, ok
}
