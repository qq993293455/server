...
type GotItem struct {
	...
	ItemId int64         `json:"f5"` // 道具id
	Num    int64         `json:"f6"` // 数量
	Msg    string        `json:"f7"` // 消息id
}

func (l *GotItem) HashKey() (dst []byte) {
	return strconv.AppendInt(dst, l.IggId, 10)
}

func (l *GotItem) ToJson() []byte {
	ret, err := json.Marshal(l)
	utils.Must(err)
	return ret
}

func (l *GotItem) GetEventType() string {
	return GotItemEventType
}

func (l *GotItem) Preset() {
	if l.EventType == "" {
