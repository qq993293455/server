...
type PlayerTitle struct {
	...
	Title  values.Level  `json:"title" gorm:"column:title;type:int(11);not null" desc:"头衔"`
}

func (p *PlayerTitle) TableName() string {
	t := p.Time.UTC().Format("20060102")
	return p.Topic() + "_" + t
}

func (p *PlayerTitle) GetRoleId() []byte {
	return utils.StringToBytes(p.RoleId)
}

func (p *PlayerTitle) Topic() string {
	return PlayerTitleTopic
}

func (p *PlayerTitle) ToJson() ([]byte, error) {
	return json.Marshal(p)
}

func (p *PlayerTitle) ToArgs() []interface{} {
	// 这里必须和结构体定义的顺序一致
	return []interface{}{
		p.Time,
		p.IggId,
...
func (p *PlayerTitle) NewModel() Model {
	return &PlayerTitle{}
}

func (p *PlayerTitle) Desc() string {
	return "玩家头衔提升"
}
