...
import (
	...
	_ "coin-server/notice-server/env"
	"coin-server/notice-server/service"
...
func main() {
	...
	serverType := models.ServerType_NoticeServer
	log := logger.MustNewAsync(zap.DebugLevel, &logger.Options{
		Console:    "stdout",
		RemoteAddr: nil,
		FilePath:   []string{fmt.Sprintf("./%s.%d.log", models.ServerType_NoticeServer.String(), serverId)},
